...
(function ($) {
  ...
  CherryJsCore.theme_script = {
    ...
    smart_slider_init: function (self) {
      $('.carprotect-smartslider').each(function () {
        var slider = $(this),
    ...
    swiper_carousel_init: function (self) {
      ...
      jQuery('.carprotect-carousel').each(function () {
        var swiper = null,
    ...
    navbar_init: function (self) {
      ...
      $(window).load(function () {
        ...
        var $layout = window.carprotect.labels.header_layout,
          $navbar = ('style-3' === $layout || 'style-7' === $layout) ? $('.vertical-menu-toggle') : $('#main-menu');

        if (!$.isFunction(jQuery.fn.stickUp) || undefined === window.carprotect.stickUp || !$navbar.length) {
          return !1;
        }
    ...
    subscribe_init: function (self) {
      CherryJsCore.variable.$document.on('click', '.subscribe-block__submit', function (event) {
        ...
        $.ajax({
          url: carprotect.ajaxurl,
          type: 'post',
          dataType: 'json',
          data: {
            action: 'carprotect_subscribe',
            mail: mail,
            nonce: nonce
          },
          error: function () {
    ...
    isMegaMenuEnabled: function () {
      ...
      if (undefined === window.carprotect.megaMenu) {
        return false;
      }

      if (true === window.carprotect.megaMenu.isActive && 'main' == window.carprotect.megaMenu.location) {
        return true;
      }
    ...
    main_menu: function (self, $mainNavigation) {
      ...
      var transitionend = 'transitionend oTransitionEnd webkitTransitionEnd',
        ...
        subMenu,
        index = -1,
        $layout = window.carprotect.labels.header_layout;

      if (hasprop.call(window, 'carprotect') &&
        hasprop.call(window.carprotect, 'more_button_options') &&
        hasprop.call(window.carprotect.more_button_options, 'more_button_type')) {
        switch (window.carprotect.more_button_options.more_button_type) {
          case 'image':
            imgurl = window.carprotect.more_button_options.more_button_image_url;
            if (window.carprotect.more_button_options.retina_more_button_image_url) {
              srcset = ' srcset="' + window.carprotect.more_button_options.retina_more_button_image_url + ' 2x"';
            }
            moreMenuContent = '<img src="' + imgurl + '"' + srcset + ' alt="' + moreMenuContent + '">';
            hasimg = true;
            break;
          case 'icon':
            moreMenuContent = '<i class="fa ' + window.carprotect.more_button_options.more_button_icon + '"></i>';
            hasicon = true;
            break;
          case 'text':
          default:
            moreMenuContent = window.carprotect.more_button_options.more_button_text || moreMenuContent;
            hasimg = false;
            hasicon = false;
            break;
        }
    ...
    to_top_init: function (self) {
      if ($.isFunction(jQuery.fn.UItoTop) && undefined !== window.carprotect.toTop) {
        $().UItoTop({
          text: carprotect.labels.totop_button,
          scrollSpeed: 600
        });
      }
    },
...
/*
     FILE ARCHIVED ON 13:29:29 Aug 25, 2018 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 16:10:28 Feb 15, 2020.
...
/*
playback timings (ms):
  LoadShardBlock: 352.086 (3)
  exclusion.robots: 0.386
  captures_list: 390.66
  RedisCDXSource: 1.081
  exclusion.robots.policy: 0.362
  PetaboxLoader3.datanode: 735.48 (4)
  CDXLines.iter: 27.357 (3)
  PetaboxLoader3.resolve: 213.632 (3)
  esindex: 0.019
  load_resource: 620.416
*/